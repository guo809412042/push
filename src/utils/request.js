...
const urlParams = (url, cleanEmptyData = true) => {
  ...
  // 筛除空字段
  const filterData = cleanEmptyData ? checkObjValue(param) : param;
  return filterData;
};
/**
 * 请求
...
const request = (url, opts = {}) => {
  const { cleanEmptyData = true, ...restOpts } = opts;
  const options = {
  ...
  let filterData = cleanEmptyData ? checkObjValue(data) : data;

  // 设置query&body
  if (method === 'GET' || method === 'DELETE') {
    const index = url.indexOf('?');
    if (index > -1) {
      filterData = urlParams(url, cleanEmptyData);
