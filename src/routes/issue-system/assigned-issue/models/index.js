...
export default {
  namespace: 'issue_system__assigned_issue',
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname }) => {
        const match = pathToRegexp('/issue_system/assigned_issue/:duiddigest?').exec(pathname);
        if (match) {
          if (match[1]) {
            dispatch({
              type: 'saveFormFields',
  ...
  effects: {
    * listInit(actions, { put, call, select }) {
      ...
      const Store = yield select(state => state.issue_system__assigned_issue);
      ...
      const issueTypeRes = yield call(getIssueTypeList, {
        productId: Store.formFields.productId, isDelete: 0,
      });
      yield put({
        type: 'saveIssueType',
        payload: {
