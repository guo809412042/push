...
export default {
  namespace: 'issue_system__automatic_issue',
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname }) => {
        const match = pathToRegexp('/issue_system/automatic_issue/:duiddigest?').exec(pathname);
        if (match) {
          if (match[1]) {
            dispatch({
              type: 'saveFormFields',
  ...
  effects: {
    * listInit(actions, { put, call, select }) {
      ...
      const Store = yield select(state => state.issue_system__automatic_issue);
