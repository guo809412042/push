...
import {
  ISSUE_SORT_ORDER_TYPE, ISSUE_STATUS, MESSAGE_STATUS, VIP_STATUS,
...
class List extends React.Component {
  ...
  handleTableChange = (pagination, filter, sorter = {}) => {
    ...
    dispatch({
      type: 'issue_system__processing_issue/savePagination',
      payload: {
        pagination: {
          current,
          pageSize,
    ...
    dispatch({
      type: 'issue_system__processing_issue/saveFormFields',
      payload: {
        data: {
          ...formFields,
          orderType,
    ...
    dispatch({ type: 'issue_system__processing_issue/listInit' });
  };
  ...
  render() {
    ...
    const column = [
      ...
      },
      {
        ...
        key: 'isNew',
        width: 100,
        render: text => <span style={{ color: String(text) === '0' ? 'red' : 'black' }}>{MESSAGE_STATUS[text] || null}</span>,
      },
      {
        title: '反馈时间',
        sorter: true,
      ...
      },
      {
        title: '处理时间',
        sorter: true,
        dataIndex: 'serviceModifyTime',
        key: 'serviceModifyTime',
        width: 120,
        render: text => (text ? moment(text).format('YYYY-MM-DD HH:mm:ss') : null),
      },
      {
      ...
      },
      {
        ...
        key: 'countryCode',
        width: 80,
        render: (text, record) => `${text || ''}--${record.lang || ''}`,
      },
      {
        title: '操作',
        width: 80,
    ...
    return (
      <>
        ...
        <div style={{
          ...
          <Button disabled={selectedRowKeys.length === 0} style={{ marginRight: 10 }} onClick={this.handleBatchReplyIssue}>批量回复</Button>
          <Button disabled={selectedRowKeys.length === 0} style={{ marginRight: 10 }} onClick={this.handleBatchDownloadIssue}>批量下载</Button>
        </div>
        <Modal
          width={900}
          title={
