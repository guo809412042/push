...
export default {
  namespace: 'issue_system__processing_issue',

  state: {
    formFields: {
      ...defaultFormFields,
      issueState: '2',
      operateName: Number(cookie.load('userid')) || '',
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname }) => {
        const match = pathToRegexp('/issue_system/processing_issue/:duiddigest?').exec(pathname);
        if (match) {
          if (match[1]) {
            dispatch({
              type: 'saveFormFields',
  ...
  effects: {
    * listInit(actions, { put, call, select }) {
      ...
      const Store = yield select(state => state.issue_system__processing_issue);
  ...
  reducers: {
    saveFormFields(state, action) {
      return { ...state, formFields: { ...defaultFormFields, ...action.payload.data } };
    },
